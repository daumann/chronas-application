...
L.Playback.Clock = L.Class.extend({
  ...
  initialize: function (trackController, callback, options) {
    ...
    this._transitionTime = 0.000000000000001; //this._tickLen / this._speed;
  },
  ...
  setSpeed: function (speed) {
    this._speed = speed;
    this._transitionTime = 0.000000000000001; //this._tickLen / speed;
    if (this._intervalID) {
      this.stop();
      this.start();
    }
...
L.Playback.DateControl = L.Control.extend({
    ...
    onAdd : function (map) {
        ...
        // date time
        this._date = L.DomUtil.create('input', 'datetimeValue', datetime);
       // this._time = L.DomUtil.create('p', '', datetime);

      //  this._date = L.DomUtil.create('input', 'slider', this._container);
        this._date.type = 'number';
        this._date.min = -10000;
        this._date.max = 2000;
        this._date.value = new Date(time).getFullYear();        
        
       //     this._date.innerHTML = new Date(time).getFullYear();//L.Playback.Util.DateStr(time);

        
     //   this._time.innerHTML = L.Playback.Util.TimeStr(time);
        
       // console.debug("!!datetime, time",datetime, time)

        // setup callback
        playback.addCallback(function (ms) {
         //   console.debug(ms,L.Playback.Util.DateStr(ms));
            var newDate = new Date(ms).getFullYear();
            ...
            //    $("g").empty()
                ...
                if(true) { //$(".datetimeValue")[0].innerHTML.substring(0,1) != "-"){
                    ...
                    newObject.properties.description = "chronasYear"+$(".datetimeValue")[0].value.substr(1)+"_BC";
                }
...
L.Playback.PlayControl = L.Control.extend({
    ...
    onAdd : function (map) {
        ...
        var self = this;
        var playback = this.playback;
        playback.setSpeed(100000000000000000);
